...
TIMER_SHIM = Path(__file__).resolve().parents[1] / "static" / "vendor" / "three-timer-shim.js"
...
def test_shared_three_core_precedes_every_dependent_console_bundle() -> None:
    ...
    timer_shim = '<script defer src="/vendor/three-timer-shim.js"></script>'
    dependents = (
    ...
    for dependent in dependents:
        assert core_position < shim_position < html.index(dependent)


def test_timer_shim_is_a_served_deferred_asset_with_required_api() -> None:
    shim = TIMER_SHIM.read_text(encoding="utf-8")

    assert TIMER_SHIM.is_file()
    assert "THREE.Timer = Timer;" in shim
    for method in (
        "getDelta",
        "getElapsed",
        "getTimescale",
        ...
        assert f"Timer.prototype.{method}" in shim
