...
/* teardown-race guard (additive, no behaviour change): regl-scatterplot's pub-sub-es
   ...
   "Cannot read properties of null (reading 'notify')". We swallow ONLY that exact vendored
   teardown error so it never reaches the operator console; every other error propagates. */
(function(){
  if(window.__szlTeardownGuard) return; window.__szlTeardownGuard=true;
  function isBenignTeardown(msg){
    msg=String(msg||'');
    return /reading 'notify'/.test(msg) || /\.notify is not a function/.test(msg);
  }
  window.addEventListener('error', function(ev){
    try{ if(ev && isBenignTeardown(ev.message)){ ev.preventDefault(); ev.stopImmediatePropagation(); return false; } }catch(e){}
  }, true);
...
const VIEWS = {
  ...
  healthtwin:{title:'Health Twin',badge:'LIVE 3D DIGITAL TWIN · PER-SUBSYSTEM HEALTH',sub:'The founder\'s flagship: pick a vessel or drone and watch its 3D twin show, in real time, which subsystems are <b>nominal</b>, <b>need-fix</b>, <b>need-upgrade</b>, <b>hacked</b> or <b>damaged</b>. Each of the six parts (hull · propulsion · comms · sensors · nav · payload) is coloured from a real telemetry model: live AIS vessels are labelled <b>live</b> (Digitraffic FI, no auth), sample drones are labelled <b>sample</b>. Subsystem health is computed with our own formulas — a <b>split-conformal</b> anomaly band (W5-3/W7-4, <b>not</b> Hoeffding), the <b>Λ</b> geometric-mean trust aggregate (<b>Conjecture 1</b>, not a theorem), and the <b>YUYAY</b> 13-axis conjunctive gate. Click a subsystem mesh for its computed metric, conformity vs envelope, and recommended action. "hacked"/"needs-fix" are <b>probabilistic inferences signed by Λ — not guarantees</b>.',
    render:async(c)=>{c.innerHTML=`<div class="kpis">
      ...
      <div class="card" id="tw-compromise"><div class="card-h"><span class="card-t">Compromise signal — kinematic · firmware · sanctions</span><span class="card-ep">real feeds · honest evidence</span></div><div class="row mono dim" style="padding:.4rem">select a platform to evaluate the live compromise cross-reference.</div></div>
      <div class="card" id="tw-remediate"><div class="card-h"><span class="card-t">Governed remediation — ROE → Λ-gate → signed receipt</span><span class="card-ep">effector SIMULATED · governance real</span></div><div class="row mono dim" style="padding:.4rem">select a platform to enable governed actions.</div></div>
...
async function kbformulas_load(){
  try{const kb=await loadKnowledge();const fm=kb.formulas||[];const pf=kb.puriq_formulas||[];const ps=kb.proof_summary||{};_kf_all=fm;
    ...
    const locked=ps.locked_proven||8;
...
async function twin_fetch(){
  ...
  twin_renderSubs(st);
  twin_renderCompromise(st);
  twin_renderRemediate(st);
  twin_applyColors(st);
}
...
function twin_renderSubs(st){
  ...
  host.innerHTML=(st.subsystems||[]).map(s=>{
    ...
    const live=(s.derived===true)||(typeof s.provenance==='string'&&s.provenance.indexOf('LIVE')===0);
    const prov=live
      ? '<span class="mono" style="color:'+LIVE+';font-size:.66rem;border:1px solid '+LIVE+';border-radius:3px;padding:0 .25rem" title="'+esc(s.provenance||'LIVE-derived')+'">LIVE-derived</span>'
      : '<span class="mono" style="color:'+WARN+';font-size:.66rem;border:1px solid '+WARN+';border-radius:3px;padding:0 .25rem" title="'+esc(s.provenance||'INFERRED/SIMULATED')+'">INFERRED/SIM</span>';
    return `<div class="lp-rail-item" style="border-left:3px solid ${c};cursor:pointer" onclick="twin_subDetail('${s.subsystem}')">
      <div class="row" style="justify-content:space-between;align-items:center"><b style="text-transform:capitalize">${esc(s.subsystem)}</b><span>${prov}&nbsp;<span class="mono" style="color:${c}">${esc(s.status)}</span></span></div>
...
function twin_subDetail(axis){
  ...
  card.innerHTML=`<div class="card-h"><span class="card-t" style="text-transform:capitalize">${esc(axis)} — <span style="color:${c}">${esc(s.status)}</span></span><span class="card-ep">${esc(st.label)} · ${esc(st.name)}</span></div>
    <div style="padding:.4rem .2rem">
      ...
      <div class="row mono" style="font-size:.78rem;padding:.5rem;margin-top:.45rem;background:${(s.derived===true)?'rgba(95,179,163,0.08)':'rgba(201,160,95,0.06)'};border-radius:6px;color:${(s.derived===true)?LIVE:WARN}"><b>Provenance:</b>&nbsp;${esc(s.provenance||((s.derived===true)?'LIVE-derived':'INFERRED/SIMULATED'))}</div>
      <div class="mono dim" style="font-size:.7rem;margin-top:.5rem">Status derived from a split-conformal band (W5-3/W7-4, NOT Hoeffding) over the live/sample telemetry; "hacked"/"needs-fix" are probabilistic inferences signed by Λ — not guarantees. COMMS/NAV/SENSORS are LIVE-derived from the real kinematic track; HULL/PROPULSION/PAYLOAD are INFERRED/SIMULATED — no public telemetry feed exposes them. Inferred is never presented as measured.</div>
    </div>`;
  if(window._twin&&window._twin.highlight) window._twin.highlight(axis);
}
window.twin_subDetail=twin_subDetail;

// ── Compromise panel: honest cross-reference (kinematic spoof + CISA KEV/NVD firmware-family
//    advisory + OFAC SDN sanctions) with the specific evidence + a real source link per check. ──
function twin_compHex(state){ return state==='compromised'?RED:(state==='watch'?GOLD:TEAL); }
function twin_renderCompromise(st){
  const host=el('tw-compromise'); if(!host)return;
  const cm=st.compromise||{};
  const state=cm.state||'clear';
  const sc=(cm.compromise_score!=null)?cm.compromise_score:0;
  const col=twin_compHex(state);
  const fired=cm.checks_fired||[];
  let rows='';
  if(!fired.length){
    rows='<div class="row mono dim" style="padding:.5rem">no compromise check fired — clear against the live kinematic, firmware-family and sanctions feeds.</div>';
  } else {
    rows=fired.map(c=>{
      const url=c.source_url||c.nvd_source_url||'';
      const isHttp=/^https?:\/\//i.test(url);
      const link=isHttp?`<a href="${esc(url)}" target="_blank" rel="noopener" style="color:${LIVE}">${esc(c.source||url)}</a>`:`<span class="mono dim">${esc(c.source||url||'live track')}</span>`;
      // CVE chips (KEV/NVD) shown verbatim from the real feed — never fabricated
      let cves='';
      const kev=(c.kev||[]).map(k=>`<span class="badge" style="border:1px solid #b06a5a;color:#d39080;font-size:.66rem" title="KEV ${esc(k.vendor||'')} ${esc(k.product||'')}">${esc(k.cve)} (KEV)</span>`);
      const nvd=(c.nvd||[]).map(n=>`<span class="badge" style="border:1px solid #7f9bd6;color:#9fb4e0;font-size:.66rem" title="NVD ${esc(n.family||'')}: ${esc(n.desc||'')}">${esc(n.cve)} (NVD)</span>`);
      const chips=kev.concat(nvd);
      if(chips.length) cves=`<div class="row" style="flex-wrap:wrap;gap:.3rem;margin-top:.35rem">${chips.join('')}</div>`;
      return `<div class="lp-rail-item" style="border-left:3px solid ${col}">
        <div class="row" style="justify-content:space-between"><b class="mono" style="color:${col}">${esc(c.check)}</b><span class="mono dim" style="font-size:.66rem">${esc(c.ts||'')}</span></div>
        <div class="mono" style="font-size:.74rem;color:#cdd2d8;line-height:1.5;margin:.25rem 0">${esc(c.evidence)}</div>
        ${cves}
        <div class="mono dim" style="font-size:.68rem;margin-top:.3rem">source: ${link}</div></div>`;
    }).join('');
  }
  const fw=cm.firmware_feeds||{};
  const fwNote=(fw.kev_available===false||fw.nvd_available===false)?`<div class="mono dim" style="font-size:.66rem;margin-top:.3rem">${esc(fw.kev_note||'')} ${esc(fw.nvd_note||'')}</div>`:'';
  host.innerHTML=`<div class="card-h"><span class="card-t">Compromise signal — kinematic · firmware · sanctions</span><span class="card-ep">real feeds · honest evidence</span></div>
    <div style="padding:.4rem .2rem">
      <div class="row" style="align-items:center;gap:.8rem;margin-bottom:.5rem">
        <span class="badge" style="border:1px solid ${col};color:${col};font-weight:600;letter-spacing:.04em">${esc(state.toUpperCase())}</span>
        <span class="mono" style="color:${col}">score ${(sc).toFixed(2)}</span>
        <span class="mono dim" style="font-size:.7rem">${fired.length} of ${(cm.checks_evaluated||[]).length} checks fired</span>
      </div>
      ${rows}
      ${fwNote}
      <div class="mono dim" style="font-size:.68rem;margin-top:.5rem;line-height:1.5">Kinematic checks are per-platform from the REAL ADS-B/AIS track. The firmware-family advisory is an ECOSYSTEM signal from the live CISA KEV + NVD feeds — NOT a claim this unit runs the vulnerable build (no public telemetry exposes per-unit firmware). Sanctions is a live OFAC SDN name screen (advisory). No CVE match is ever fabricated — only real feed hits are shown.</div>
    </div>`;
}
window.twin_renderCompromise=twin_renderCompromise;

// ── Governed remediation: POST /twin/remediate {platform, action} → ROE → Λ-gate → signed
//    receipt. The effector is honestly SIMULATED; the governance + receipt are real. ──
const TWIN_ACTIONS=[
  {a:'upgrade', l:'Upgrade', d:'push a firmware/capability upgrade (OTA)'},
  {a:'patch',   l:'Patch',   d:'apply a security patch (OTA)'},
  {a:'recall',  l:'Recall',  d:'recall the asset to base'},
  {a:'isolate', l:'Isolate', d:'quarantine a compromised link (containment)'}
];
function twin_renderRemediate(st){
  const host=el('tw-remediate'); if(!host)return;
  const pid=st.platform_id, nm=st.name||pid;
  const btns=TWIN_ACTIONS.map(x=>`<button class="btn" onclick="twin_remediate('${esc(pid)}','${x.a}')" title="${esc(x.d)}" style="border:1px solid #2a2d33">${x.l}</button>`).join('');
  host.innerHTML=`<div class="card-h"><span class="card-t">Governed remediation — ROE → Λ-gate → signed receipt</span><span class="card-ep">effector SIMULATED · governance real</span></div>
    <div style="padding:.4rem .2rem">
...
async function twin_remediate(pid, action){
  const out=el('tw-remout'); if(out)out.innerHTML='<div class="row mono dim" style="padding:.4rem">running governed '+esc(action)+'…</div>';
  let r;
  try{ r=await postJSON('/api/killinchu/v1/twin/remediate',{platform:pid,action:action}); }
  catch(e){ if(out)out.innerHTML='<div class="row mono" style="color:'+RED+';padding:.4rem">remediate failed: '+esc(e.message)+'</div>'; return; }
  if(!out)return;
  if(r.error){ out.innerHTML='<div class="row mono" style="color:'+RED+';padding:.4rem">'+esc(r.error)+'</div>'; return; }
  const g=r.governance||{};
  const dec=r.decision||g.decision||'—';
  const ok=(dec==='AUTHORIZED');
  const dcol=ok?TEAL:RED;
  const roe=(g.roe||[]).map(x=>`<div class="row mono" style="font-size:.72rem;gap:.5rem"><span style="color:${x.pass?TEAL:RED}">${x.pass?'✓':'✗'}</span><b>${esc(x.rule)}</b><span class="dim">${esc(x.detail||'')}</span></div>`).join('');
  const rc=r.receipt||{};
  const dsse=rc.dsse?JSON.stringify(rc.dsse,null,2):'';
  const recBlock=rc.index!=null?`<div class="row mono" style="font-size:.74rem;gap:1rem;margin-top:.4rem"><span><b style="color:#c9b787">receipt #</b>${esc(String(rc.index))}</span><span><b style="color:#c9b787">digest</b> ${esc(String(rc.digest||'').slice(0,16))}…</span><span><b style="color:#c9b787">signed</b> <span style="color:${rc.signed?TEAL:WARN}">${rc.signed?'ECDSA-P256':'UNSIGNED (no fabricated sig)'}</span></span></div>`+(dsse?`<pre class="mono" style="background:#0f1216;border:1px solid #23262c;border-radius:6px;padding:.5rem;font-size:.68rem;color:#cdd2d8;overflow:auto;max-height:150px;margin-top:.35rem">${esc(dsse)}</pre>`:''):`<div class="mono dim" style="font-size:.7rem;margin-top:.35rem">${esc(rc.note||'no receipt node')}</div>`;
  out.innerHTML=`<div class="card" style="border:1px solid ${dcol};background:#0f1216">
    <div class="row" style="justify-content:space-between;padding:.5rem .6rem;border-bottom:1px solid #23262c">
      <b class="mono" style="color:${dcol};letter-spacing:.04em">${esc(action.toUpperCase())} → ${esc(dec)}</b>
      <span class="mono dim" style="font-size:.7rem">Λ=${g.lambda!=null?Number(g.lambda).toFixed(4):'—'} · compromise ${esc(g.compromise_state||'—')}</span></div>
    <div style="padding:.5rem .6rem">
      <div class="mono" style="font-size:.74rem;margin-bottom:.4rem"><b style="color:#c9b787">Intent:</b> ${esc(g.intent||'')}</div>
      <div class="mono" style="font-size:.72rem;color:#9a9a9a;margin-bottom:.25rem"><b>Rules of Engagement</b> (roe_pass=${g.roe_pass}):</div>
      ${roe}
      <div class="mono" style="font-size:.72rem;margin-top:.4rem"><b style="color:#c9b787">Λ-gate:</b> <span style="color:${(g.lambda_gate&&g.lambda_gate.authorized)?TEAL:RED}">${(g.lambda_gate&&g.lambda_gate.authorized)?'PASS':'DENY'}</span> <span class="dim">${esc((g.lambda_gate&&g.lambda_gate.rule)||'')}</span></div>
      <div class="mono" style="font-size:.72rem;margin-top:.3rem"><b style="color:#c9b787">Lean ref:</b> <span class="dim">${esc(g.lean_theorem_ref?JSON.stringify(g.lean_theorem_ref):'—')}</span></div>
      ${recBlock}
      <div class="mono dim" style="font-size:.68rem;margin-top:.45rem;line-height:1.5">${esc(g.effector||'')}</div>
      ${r.receipt_honesty?`<div class="mono dim" style="font-size:.66rem;margin-top:.3rem">${esc(r.receipt_honesty)}</div>`:''}
    </div></div>`;
}
window.twin_remediate=twin_remediate;
...
(function(){
  ...
  function statCol(s){ return s==='LIVE'?TEAL:(s==='CI-GREEN'?GOLD:(s==='LIVE-GATED'?WARN:DIM)); }
  var CAPS=[
    {k:'rag', n:'Agentic Lineage-Aware RAG', s:'LIVE',
     p:'Retrieval that carries provenance/lineage into the gate, so untrusted context provably cannot flip DENY\u2192ALLOW (Goguen\u2013Meseguer non-interference, P3). The reasoning cortex reads frozen snapshots \u2014 never writes the record.',
     e:'szl_rag.py (a11oy superset, shared w/ killinchu) \u00b7 YACHAY temporal retrieval', f:'P3, P1'},
    {k:'mcp', n:'Agentic MCP tool surface (hatun-mcp)', s:'LIVE',
     p:'The canonical SZL formula registry + flagship ops as agentic MCP tools (list/run/proof_status, a11oy_gate, lambda_score, khipu_sign/verify; killinchu detect/evaluate/cue/halt_drone). proof_status passed VERBATIM, never inflated. Live: 11 tools (7 a11oy + 4 killinchu).',
     e:'GET /api/a11oy/v1/mcp/tools \u2192 count:11 \u00b7 hatun-mcp adapter', f:'Ph1'},
    {k:'ouroboros', n:'Ouroboros governed loop', s:'CI-GREEN',
  ...
  function render(host){
    ...
    CAPS.forEach(function(c){
      h+='<div style="border:1px solid #232830;border-radius:9px;padding:12px 14px;margin-bottom:10px;background:#0e1217">'
        ...
        +'<span style="font-weight:700;color:#e6eaee;font-size:14px">'+esc(c.n)+'</span>'+pill(c.s,statCol(c.s))+'</div>'
        +'<div style="font-size:12.5px;color:#b6bcc4;margin-top:7px;line-height:1.55">'+esc(c.p)+'</div>'
        +'<div style="font-family:var(--mono,monospace);font-size:10.5px;color:'+DIM+';margin-top:8px">evidence: '+esc(c.e)+'</div>'
        +'<div style="font-family:var(--mono,monospace);font-size:10.5px;color:'+GOLD+';margin-top:3px">formulas: '+esc(c.f)+'</div>'
        +'</div>';
    });
    h+='<div style="font-family:var(--mono,monospace);font-size:10px;color:'+DIM+';margin-top:6px">'
      +'Static honest snapshot \u00b7no fabricated live pills \u00b7 statuses reflect real surface state at snapshot time.</div>';
    host.innerHTML=h;
  }
  function reg(){
